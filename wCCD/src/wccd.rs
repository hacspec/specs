...
// #[cfg(not(feature = "hacspec"))]
/// The contract state,
#[hax::contract_state(contract = "CIS1-wCCD")]
#[derive(Serialize, SchemaType, Clone)]
struct State {
...
/// Initialize contract instance with no initial tokens.
/// Logs a `Mint` event for the single token id with no amounts.
#[hax::init(contract = "CIS1-wCCD", enable_logger)]
...
fn contract_wrap<A: HasActions>(
    ...
    amount: Amount,
    logger: &impl HasLogger,
    state: State,
) -> Result<(A, State), cis1::Cis1Error<CustomContractError>> {
    ...
    if let cis1::Receiver::Contract(address, function) = params.to {
        ...
        Ok((A::send_raw(&address, function.as_ref(), Amount::zero(), &param_bytes), state_ret))
    } else {
...
fn contract_unwrap<A: HasActions>(
    ...
    let action = match params.receiver {
        ...
        cis1::Receiver::Contract(address, function) => {
            A::send_raw(&address, function.as_ref(), unwrapped_amount, params.data.as_ref())
        }
    };
...
fn contract_transfer<A: HasActions>(
    ...
    for _ in 0..transfers_length {
        ...
        if let cis1::Receiver::Contract(address, function) = to {
            ...
            // let action = hacspec_concordium::send(&address, function.as_ref(), Amount::zero(), parameter.data.as_ref()); // Was &parameter
            let action = A::accept();
            actions = actions.and_then(action);
        }
    }
    let mut state_ret = state.clone();
    Ok((actions,state_ret))
}
...
#[hax::receive(contract = "CIS1-wCCD", name = "balanceOf", parameter = "ContractBalanceOfQueryParams")]
...
/// - It fails to parse the parameter.
/// - Message sent back with the result rejects.
#[hax::receive(contract = "CIS1-wCCD", name = "operatorOf", parameter = "OperatorOfQueryParams")]
...
fn contract_token_metadata<A: HasActions>(
    ...
    let mut state_ret = _state.clone();
    // Send back the response.
    Ok((A::send_raw(
        &result_contract,
        result_hook.as_ref(),
