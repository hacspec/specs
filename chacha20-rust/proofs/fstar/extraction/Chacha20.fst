...
let t_State = t_Array u32 16sz

let t_Block = t_Array u8 64sz

let t_ChaChaIV = t_Array u8 12sz

let t_ChaChaKey = t_Array u8 32sz

let t_StateIdx = Bounded_indexes.t_BoundedIndex 16sz

let chacha20_line (a b d: Bounded_indexes.t_BoundedIndex 16sz) (s: u32) (m: t_Array u32 16sz)
    : t_Array u32 16sz =
  let state:t_Array u32 16sz = m in
  let state:t_Array u32 16sz =
    state.[ a ] <-
      Core.Num.wrapping_add_under_impl_8 (state.[ a ] <: u32) (state.[ b ] <: u32) <: u32
  in
  let state:t_Array u32 16sz = state.[ d ] <- (state.[ d ] <: u32) ^. (state.[ a ] <: u32) <: u32 in
  let state:t_Array u32 16sz =
    state.[ d ] <- Core.Num.rotate_left_under_impl_8 (state.[ d ] <: u32) s <: u32
  in
  state

let chacha20_quarter_round (a b c d: Bounded_indexes.t_BoundedIndex 16sz) (state: t_Array u32 16sz)
    : t_Array u32 16sz =
  let state:t_Array u32 16sz = chacha20_line a b d 16ul state in
  let state:t_Array u32 16sz = chacha20_line c d b 12ul state in
  let state:t_Array u32 16sz = chacha20_line a b d 8ul state in
  chacha20_line c d b 7ul state

let chacha20_double_round (state: t_Array u32 16sz) : t_Array u32 16sz =
  let state:t_Array u32 16sz =
...
let chacha20_rounds (state: t_Array u32 16sz) : t_Array u32 16sz =
  let st:t_Array u32 16sz = state in
  let st:t_Array u32 16sz =
    Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter ({
        ...
        _)
      st
      (fun st v__i -> chacha20_double_round st <: t_Array u32 16sz)
  in
  st

let chacha20_core (ctr: u32) (st0: t_Array u32 16sz) : t_Array u32 16sz =
  let state:t_Array u32 16sz = st0 in
  let state:t_Array u32 16sz =
    state.[ 12sz ] <- Core.Num.wrapping_add_under_impl_8 (state.[ 12sz ] <: u32) ctr <: u32
  in
  let k:t_Array u32 16sz = chacha20_rounds state in
  Chacha20.Hacspec_helper.add_state state k

let chacha20_init (key: t_Array u8 32sz) (iv: t_Array u8 12sz) (ctr: u32) : t_Array u32 16sz =
  let (key_u32: t_Array u32 8sz):t_Array u32 8sz =
    Chacha20.Hacspec_helper.to_le_u32s_8_ (Rust_primitives.unsize key <: slice u8)
  in
  let (iv_u32: t_Array u32 3sz):t_Array u32 3sz =
...
let chacha20_key_block (state: t_Array u32 16sz) : t_Array u8 64sz =
  let state:t_Array u32 16sz = chacha20_core 0ul state in
  Chacha20.Hacspec_helper.u32s_to_le_bytes state

let chacha20_key_block0 (key: t_Array u8 32sz) (iv: t_Array u8 12sz) : t_Array u8 64sz =
  let state:t_Array u32 16sz = chacha20_init key iv 0ul in
  chacha20_key_block state

let chacha20_encrypt_block (st0: t_Array u32 16sz) (ctr: u32) (plain: t_Array u8 64sz)
    : t_Array u8 64sz =
  let st:t_Array u32 16sz = chacha20_core ctr st0 in
  let (pl: t_Array u32 16sz):t_Array u32 16sz =
    Chacha20.Hacspec_helper.to_le_u32s_16_ (Rust_primitives.unsize plain <: slice u8)
  in
  let encrypted:t_Array u32 16sz = Chacha20.Hacspec_helper.xor_state st pl in
  Chacha20.Hacspec_helper.u32s_to_le_bytes encrypted

let chacha20_encrypt_last (st0: t_Array u32 16sz) (ctr: u32) (plain: slice u8)
    : Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
  let (b: t_Array u8 64sz):t_Array u8 64sz = Rust_primitives.Hax.repeat 0uy 64sz in
  let b:t_Array u8 64sz = Chacha20.Hacspec_helper.update_array b plain in
  let b:t_Array u8 64sz = chacha20_encrypt_block st0 ctr b in
...
let chacha20_update (st0: t_Array u32 16sz) (m: slice u8) : Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
...
let chacha20 (m: slice u8) (key: t_Array u8 32sz) (iv: t_Array u8 12sz) (ctr: u32)
    : Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global =
  let state:t_Array u32 16sz = chacha20_init key iv ctr in
