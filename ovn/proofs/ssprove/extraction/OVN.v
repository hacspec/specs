  ...
  #[program] Definition Exec_i_realised_code m (i j : pid) (vote : 'bool):
    ...
    {code
     ...
     | Some a =>
         ...
         v0 ← get skey_loc i ;;
         v1 ← get ckey_loc i ;;
         @ret 'public (fto (expgn_rec (T:=gT) (otf v1) v0 * expgn_rec (T:=gT) g vote)%g)
         | None =>
  ...
  Proof.
    ...
    - ssprove_swap_seq_lhs [:: 0 ; 1 ; 2 ; 3 ; 4 ; 5]%N.
      ...
      + ssprove_swap_seq_lhs [:: 0 ; 1]%N.
        ...
        set zk := (fto (g ^+ x), fto (g ^+ otf r_j), s1, fto (otf x2 + otf s1 * otf x)%R).
        clearbody zk.
        specialize (Hf zk).
        rewrite !Hord.
        rewrite Hf.
      ...
      + ssprove_swap_seq_lhs [:: 0 ; 1 ; 2 ; 3]%N.
        ...
        rewrite otf_fto.
        rewrite compute_key_set_i.
        set zk := (fto (g ^+ x), fto (g ^+ otf r_j), e_j, fto (otf x2 + otf e_j * otf x)%R).
        clearbody zk.
        specialize (Hf zk).
        rewrite !Hord.
        rewrite Hf.
    ...
    - ssprove_swap_seq_lhs [:: 0 ; 1 ; 2 ; 3 ; 4 ; 5  ; 6 ; 7]%N.
      ...
      + ssprove_swap_seq_lhs [:: 0 ; 1]%N.
        ...
        rewrite otf_fto.
        rewrite compute_key_set_i.
        set zk := (fto (g ^+ x), fto (g ^+ otf r_j), s, fto (otf x2 + otf s * otf x)%R).
        clearbody zk.
        specialize (Hf zk).
        rewrite !Hord.
        rewrite Hf.
