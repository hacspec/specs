...
#[global] Program Instance t_z_17__t_Group : t_Group t_z_17_ :=
  ...
  let g_pow := fun  {L1 : {fset Location}} {I1 : Interface} (x : both L1 I1 (int32)) => solve_lift ((g .^ x) .% q) : both (L1 :|: fset []) I1 (int32) in
  let pow := fun  {L1 : {fset Location}} {L2 : {fset Location}} {I1 : Interface} {I2 : Interface} (g : both L1 I1 (int32)) (x : both L2 I2 (int32)) => solve_lift ((g .^ x) .% q) : both (L1 :|: L2 :|: fset []) (I1 :|: I2) (int32) in
  let one := fun  {L : {fset Location}} {I : Interface} => solve_lift (ret_both (1 : int32)) : both (L :|: fset []) I (int32) in
  let prod := fun  {L1 : {fset Location}} {L2 : {fset Location}} {I1 : Interface} {I2 : Interface} (x : both L1 I1 (int32)) (y : both L2 I2 (int32)) => solve_lift ((x .* y) .% q) : both (L1 :|: L2 :|: fset []) (I1 :|: I2) (int32) in
  let inv := fun  {L1 : {fset Location}} {I1 : Interface} (x : both L1 I1 (int32)) => letb res loc(res_loc) := ret_both (0 : int32) in
  letb res := foldi_both_list (into_iter (Build_t_Range (f_start := ret_both (1 : int32)) (f_end := q))) (fun i =>
    ssp (fun res =>
      letb i_computation := i in
      solve_lift (ifb (g_pow i) =.? x
      then letb res := i_computation in
        res
...
Definition register_vote_state_ret_loc {A : _} {T : _} `{ t_Sized (A)} `{ t_Sized (T)} `{ t_HasActions (A)} `{ t_HasReceiveContext (T) ('unit)} `{ t_Sized (A)} `{ t_Sized (T)} `{ t_HasActions (A)} `{ t_HasReceiveContext (T) ('unit)} : Location :=
  (t_OvnContractState ; 1%nat).
Equations register_vote {L1 : {fset Location}} {L2 : {fset Location}} {I1 : Interface} {I2 : Interface} {A : _} {T : _} `{ t_Sized (A)} `{ t_Sized (T)} `{ t_HasActions (A)} `{ t_HasReceiveContext (T) ('unit)} `{ t_Sized (A)} `{ t_Sized (T)} `{ t_HasActions (A)} `{ t_HasReceiveContext (T) ('unit)} (ctx : both L1 I1 (T)) (state : both L2 I2 (t_OvnContractState)) : both (L1 :|: L2 :|: fset [register_vote_state_ret_loc]) (I1 :|: I2) (t_Result ((A × t_OvnContractState)) (t_ParseError)) :=
  register_vote ctx state  :=
    solve_lift (run (letb '(_,out) := get (parameter_cursor ctx) in
    letm[choice_typeMonad.result_bind_code (t_Result ((A × t_OvnContractState)) (t_ParseError))] (params:t_RegisterParam) := matchb branch out with
    | ControlFlow_Break residual =>
      letm[choice_typeMonad.result_bind_code (t_Result ((A × t_OvnContractState)) (t_ParseError))] hoist1 := v_Break (from_residual residual) in
      ControlFlow_Continue (never_to_any hoist1)
    | ControlFlow_Continue val =>
      ControlFlow_Continue val
    end in
    ControlFlow_Continue (letb g_pow_xi := g_pow (f_rp_xi params) in
    letb zkp_xi := v_ZKP g_pow_xi (f_rp_xi params) in
    letb register_vote_state_ret loc(register_vote_state_ret_loc) := clone state in
    ...
    Result_Ok (prod_b (accept,register_vote_state_ret))))) : both (L1 :|: L2 :|: fset [register_vote_state_ret_loc]) (I1 :|: I2) (t_Result ((A × t_OvnContractState)) (t_ParseError)).
Fail Next Obligation.
...
Definition prod2_loc : Location :=
  (int32 ; 3%nat).
Equations compute_group_element_for_vote {L1 : {fset Location}} {L2 : {fset Location}} {L3 : {fset Location}} {L4 : {fset Location}} {I1 : Interface} {I2 : Interface} {I3 : Interface} {I4 : Interface} (i : both L1 I1 (int32)) (xi : both L2 I2 (int32)) (vote : both L3 I3 ('bool)) (xis : both L4 I4 (nseq int32 20)) : both (L1 :|: L2 :|: L3 :|: L4 :|: fset [prod1_loc;prod2_loc;res_loc]) (I1 :|: I2 :|: I3 :|: I4) (int32) :=
  compute_group_element_for_vote i xi vote xis  :=
    letb prod1 loc(prod1_loc) := one in
    letb prod1 := foldi_both_list (into_iter (Build_t_Range (f_start := ret_both (0 : uint_size)) (f_end := cast_int (WS2 := _) (i .- (ret_both (1 : int32)))))) (fun j =>
      ssp (fun prod1 =>
        solve_lift (prod prod1 (xis.a[j])) : both (*2*)(L1:|:L4:|:L1:|:fset [prod1_loc;prod1_loc]) (I1:|:I4:|:I1) (t_group_type))) prod1 in
    letb prod2 loc(prod2_loc) := one in
    letb prod2 := foldi_both_list (into_iter (Build_t_Range (f_start := cast_int (WS2 := _) (i .+ (ret_both (1 : int32)))) (f_end := n))) (fun j =>
      ssp (fun prod2 =>
        solve_lift (prod prod2 (xis.a[j])) : both (*2*)(L1:|:L4:|:L1:|:fset [prod2_loc;prod2_loc]) (I1:|:I4:|:I1) (t_group_type))) prod2 in
    letb Yi := div prod1 prod2 in
    solve_lift (prod (pow Yi xi) (g_pow (ifb vote
    then ret_both (1 : int32)
    else ret_both (0 : int32)))) : both (L1 :|: L2 :|: L3 :|: L4 :|: fset [prod1_loc;prod2_loc;res_loc]) (I1 :|: I2 :|: I3 :|: I4) (int32).
Fail Next Obligation.
...
Definition commit_to_vote_state_ret_loc {A : _} {(* impl *) HasReceiveContext : _} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} : Location :=
  (t_OvnContractState ; 4%nat).
Equations commit_to_vote {L1 : {fset Location}} {L2 : {fset Location}} {I1 : Interface} {I2 : Interface} {A : _} {(* impl *) HasReceiveContext : _} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} (ctx : both L1 I1 ((* impl *) HasReceiveContext)) (state : both L2 I2 (t_OvnContractState)) : both (L1 :|: L2 :|: fset [commit_to_vote_state_ret_loc;prod1_loc;prod2_loc;res_loc]) (I1 :|: I2) (t_Result ((A × t_OvnContractState)) (t_ParseError)) :=
  commit_to_vote ctx state  :=
    solve_lift (run (letb '(_,out) := get (parameter_cursor ctx) in
    letm[choice_typeMonad.result_bind_code (t_Result ((A × t_OvnContractState)) (t_ParseError))] (params:t_CastVoteParam) := matchb branch out with
    | ControlFlow_Break residual =>
      letm[choice_typeMonad.result_bind_code (t_Result ((A × t_OvnContractState)) (t_ParseError))] hoist2 := v_Break (from_residual residual) in
      ControlFlow_Continue (never_to_any hoist2)
    | ControlFlow_Continue val =>
      ControlFlow_Continue val
    end in
    ControlFlow_Continue (letb _ := foldi_both_list (into_iter (f_zkp_xis state)) (fun zkp =>
      ssp (fun _ =>
    ...
    letb commit_to_vote_state_ret loc(commit_to_vote_state_ret_loc) := clone state in
    ...
    Result_Ok (prod_b (accept,commit_to_vote_state_ret))))) : both (L1 :|: L2 :|: fset [commit_to_vote_state_ret_loc;prod1_loc;prod2_loc;res_loc]) (I1 :|: I2) (t_Result ((A × t_OvnContractState)) (t_ParseError)).
Fail Next Obligation.
...
Definition cast_vote_state_ret_loc {A : _} {(* impl *) HasReceiveContext : _} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} : Location :=
  (t_OvnContractState ; 5%nat).
Equations cast_vote {L1 : {fset Location}} {L2 : {fset Location}} {I1 : Interface} {I2 : Interface} {A : _} {(* impl *) HasReceiveContext : _} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} (ctx : both L1 I1 ((* impl *) HasReceiveContext)) (state : both L2 I2 (t_OvnContractState)) : both (L1 :|: L2 :|: fset [cast_vote_state_ret_loc;prod1_loc;prod2_loc;res_loc]) (I1 :|: I2) (t_Result ((A × t_OvnContractState)) (t_ParseError)) :=
  cast_vote ctx state  :=
    solve_lift (run (letb '(_,out) := get (parameter_cursor ctx) in
    letm[choice_typeMonad.result_bind_code (t_Result ((A × t_OvnContractState)) (t_ParseError))] (params:t_CastVoteParam) := matchb branch out with
    | ControlFlow_Break residual =>
      letm[choice_typeMonad.result_bind_code (t_Result ((A × t_OvnContractState)) (t_ParseError))] hoist3 := v_Break (from_residual residual) in
    ...
    letb cast_vote_state_ret loc(cast_vote_state_ret_loc) := clone state in
    ...
    Result_Ok (prod_b (accept,cast_vote_state_ret))))) : both (L1 :|: L2 :|: fset [cast_vote_state_ret_loc;prod1_loc;prod2_loc;res_loc]) (I1 :|: I2) (t_Result ((A × t_OvnContractState)) (t_ParseError)).
Fail Next Obligation.
...
Definition tally_loc {A : _} {(* impl *) HasReceiveContext : _} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} : Location :=
  (int32 ; 6%nat).
Definition tally_votes_state_ret_loc {A : _} {(* impl *) HasReceiveContext : _} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} : Location :=
  (t_OvnContractState ; 7%nat).
Definition vote_result_loc {A : _} {(* impl *) HasReceiveContext : _} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} : Location :=
  (t_group_type ; 8%nat).
Equations tally_votes {L1 : {fset Location}} {L2 : {fset Location}} {I1 : Interface} {I2 : Interface} {A : _} {(* impl *) HasReceiveContext : _} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} `{ t_Sized (A)} `{ t_Sized ((* impl *) HasReceiveContext)} `{ t_HasActions (A)} `{ t_HasReceiveContext ((* impl *) HasReceiveContext) ('unit)} (_ : both L1 I1 ((* impl *) HasReceiveContext)) (state : both L2 I2 (t_OvnContractState)) : both (L1 :|: L2 :|: fset [tally_loc;tally_votes_state_ret_loc;vote_result_loc]) (I1 :|: I2) (t_Result ((A × t_OvnContractState)) (t_ParseError)) :=
  tally_votes _ state  :=
    letb _ := foldi_both_list (into_iter (Build_t_Range (f_start := ret_both (0 : uint_size)) (f_end := n))) (fun i =>
      ssp (fun _ =>
        ...
        solve_lift (ret_both (tt : 'unit)) : both (*0*)(L2:|:fset []) (I2) ('unit))) (ret_both (tt : 'unit)) in
    letb (vote_result) loc(vote_result_loc) := one in
    letb vote_result := foldi_both_list (into_iter (f_g_pow_xi_yi_vis state)) (fun g_pow_vote =>
      ssp (fun vote_result =>
        solve_lift (prod vote_result g_pow_vote) : both (*2*)(L2:|:L2:|:fset [vote_result_loc;vote_result_loc]) (I2:|:I2) (t_group_type))) vote_result in
    letb tally loc(tally_loc) := ret_both (0 : int32) in
    letb tally := foldi_both_list (into_iter (Build_t_Range (f_start := ret_both (0 : int32)) (f_end := cast_int (WS2 := _) n))) (fun i =>
      ssp (fun tally =>
    ...
    letb tally_votes_state_ret loc(tally_votes_state_ret_loc) := clone state in
    letb tally_votes_state_ret := Build_t_OvnContractState[tally_votes_state_ret] (f_tally := tally) in
    Result_Ok (solve_lift (prod_b (accept,tally_votes_state_ret))) : both (L1 :|: L2 :|: fset [tally_loc;tally_votes_state_ret_loc;vote_result_loc]) (I1 :|: I2) (t_Result ((A × t_OvnContractState)) (t_ParseError)).
Fail Next Obligation.
