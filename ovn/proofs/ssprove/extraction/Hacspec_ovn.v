...
Class t_Group (Self : choice_type) := {
  ...
  prod : forall {L1 L2 I1 I2}, both L1 I1 t_group_type -> both L2 I2 t_group_type -> both (L1 :|: L2) (I1 :|: I2) t_group_type ;
  div : forall {L1 L2 I1 I2}, both L1 I1 t_group_type -> both L2 I2 t_group_type -> both (L1 :|: L2) (I1 :|: I2) t_group_type ;
  random_element : t_group_type ;
}.
...
Equations get_broadcast1 : both (fset []) ([interface ]) ((t_Vec (uint_size) (t_Global) × t_Vec (uint_size) (t_Global))) :=
  get_broadcast1  :=
    solve_lift (prod_b (@new _ fset0 fset0,@new _ fset0 fset0)) : both (fset []) ([interface ]) ((t_Vec (uint_size) (t_Global) × t_Vec (uint_size) (t_Global))).
Fail Next Obligation.
...
Equations register_vote (G : _) (* `{ t_Sized (G)} *) `{ t_Group (G)} {L1 : {fset Location}} {L2 : {fset Location}} {I1 : Interface} {I2 : Interface} (i : both L1 I1 (uint_size)) (random : both L2 I2 (uint_size)) : both (L1:|:L2 :|: fset [prod1_loc]) (I1:|:I2) ('unit) :=
  register_vote G i random  := 
    ...
    (* letb _ := (other loop todo(term)) : both _ _ ('unit) in *) (* TODO LOOP *)
...
Equations get_broadcast2 (G : _) (* `{ t_Sized (G)} *) `{ t_Group (G)} : both (fset []) ([interface ]) ((t_Vec (t_group_type) (t_Global) × t_Vec (t_group_type) (t_Global) × t_Vec (uint_size) (t_Global))) :=
  get_broadcast2 G  :=
    solve_lift (prod_b (@new _ fset0 fset0,@new _ fset0 fset0,@new _ fset0 fset0)) : both (fset []) ([interface ]) ((t_Vec (t_group_type) (t_Global) × t_Vec (t_group_type) (t_Global) × t_Vec (uint_size) (t_Global))).
Fail Next Obligation.
...
Definition tally_loc {G : _} (* `{ t_Sized (G)} *) `{ t_Group (G)} : Location :=
  (uint_size ; 1%nat).
Definition into_iter_zip {L I A B} (x : both L I (chList (A × B))) : both L I (chList A) * both L I (chList B). Admitted.
Equations tally_votes (G : _) (* `{ t_Sized (G)} *) `{ t_Group (G)} : both (fset [tally_loc; vote_result_loc]) ([interface ]) (uint_size) :=
  tally_votes G  :=
    ...
    (* letb vote_result := (foldi_both (into_iter_zip ((lift2_both (fun (x : chList _) (y : chList _) => seq.zip x y : chList (chProd _ _))) (into_iter g_pow_xi_yi) g_pow_vi)) (fun {L I _ _} =>fun '(gxiyi,gvi) => *)
    (*     (ssp (fun vote_result => *)
      (*       solve_lift (prod vote_result (prod gxiyi gvi))) )) vote_result) : both _ _ (t_group_type) in *)
      (* TODO for_loop *)
