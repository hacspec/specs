...
Definition ch (x : t_U64_t) (y : t_U64_t) (z : t_U64_t) : _ :=
  (x.&y).^((not x).&z).

Definition maj (x : t_U64_t) (y : t_U64_t) (z : t_U64_t) : _ :=
  (x.&y).^((x.&z).^(y.&z)).
...
Definition sigma (x : t_U64_t) (i : uint_size) (op : uint_size) : _ :=
  let tmp := (rotate_right_under_impl_97 x (v_OP_TABLE.[(((@repr WORDSIZE32 3).*i).+(@repr WORDSIZE32 2))])) : t_U64_t in
  let tmp := (if
    ...
    then
      ...
      tmp) : _ in
  ((rotate_right_under_impl_97 x (v_OP_TABLE.[((@repr WORDSIZE32 3).*i)])).^(rotate_right_under_impl_97 x (v_OP_TABLE.[(((@repr WORDSIZE32 3).*i).+(@repr WORDSIZE32 1))]))).^tmp.

Definition schedule (block : t_Block_t) : t_RoundConstantsTable_t :=
  let b := (to_be_U64s_under_impl block) : t_Seq_t t_U64_t in
...
Definition hash (msg : t_Seq_t t_U8_t) : t_Sha512Digest_t :=
  ...
  let len_bist := (U128 (cast ((len_under_impl_41 msg).*(@repr WORDSIZE32 8)))) : t_U128_t in
  let '(h,last_block) := (if
      last_block_len<.?(v_BLOCK_SIZE.-v_LEN_SIZE)
    then
      let last_block := (update last_block (v_BLOCK_SIZE.-v_LEN_SIZE) (v_U128_to_be_bytes len_bist)) : t_Block_t in
      let h := (compress last_block h) : t_Hash_t in
      (h,last_block)
    else
      let pad_block := (new_under_impl_5) : t_Block_t in
      let pad_block := (update pad_block (v_BLOCK_SIZE.-v_LEN_SIZE) (v_U128_to_be_bytes len_bist)) : t_Block_t in
      let h := (compress last_block h) : t_Hash_t in
