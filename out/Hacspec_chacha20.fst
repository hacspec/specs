...
let chacha20_line (a b d s: usize) (m: t_State) : t_State =
  ...
  let state:t_State = Rust_primitives.Hax.update_at state a (state.[ a ] +. state.[ b ]) in
  let state:t_State = Rust_primitives.Hax.update_at state d (state.[ d ] ^. state.[ a ]) in
  let state:t_State =
    Rust_primitives.Hax.update_at state d (Secret_integers.rotate_left_under_impl_66 state.[ d ] s)
  in
  state
...
let chacha20_core (ctr: Secret_integers.t_U32) (st0: t_State) : _ =
  let state:t_State = st0 in
  let state:t_State = Rust_primitives.Hax.update_at state 12l (state.[ 12l ] +. ctr) in
...
let chacha20_init (key: t_ChaChaKey) (iv: t_ChaChaIV) (ctr: Secret_integers.t_U32) : t_State =
  ...
  let st:t_State = Hacspec_lib.Traits.SeqTrait.update st 0sz chacha20_constants_init in
  let st:t_State = Hacspec_lib.Traits.SeqTrait.update st 4sz (to_le_U32s_under_impl_138 key) in
  let st:t_State = Rust_primitives.Hax.update_at st 12l ctr in
  let st:t_State = Hacspec_lib.Traits.SeqTrait.update st 13sz (to_le_U32s_under_impl_103 iv) in
  st
...
let chacha20_encrypt_block (st0: t_State) (ctr: Secret_integers.t_U32) (plain: t_Block) : t_Block =
  ...
  let pl:t_State = from_seq_under_impl_17 (to_le_U32s_under_impl_68 plain) in
  let st = pl ^. st in
...
let chacha20_encrypt_last
  ...
  slice_under_impl_74 b 0sz (Hacspec_lib.Seq.len_under_impl_41 plain)
...
let chacha20_update (st0: t_State) (m: Hacspec_lib.Seq.t_Seq Secret_integers.t_U8)
  ...
  let blocks_out:Hacspec_lib.Seq.t_Seq Secret_integers.t_U8 =
    Hacspec_lib.Seq.new_under_impl_41 (Hacspec_lib.Seq.len_under_impl_41 m)
  in
      ...
      (fun blocks_out i ->
          ...
          let b:t_Block =
            chacha20_encrypt_block st0
              ...
              (from_seq_under_impl_86 msg_block)
          in
          let blocks_out:Hacspec_lib.Seq.t_Seq Secret_integers.t_U8 =
  ...
  in
  let blocks_out:Hacspec_lib.Seq.t_Seq Secret_integers.t_U8 =
    if Hacspec_lib.Seq.len_under_impl_41 last_block <>. 0sz
    then
