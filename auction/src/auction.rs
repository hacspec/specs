...
#[hax::contract_state(contract = "auction")]
...
#[hax::init(contract = "auction", parameter = "InitParameter")]
fn auction_init(ctx: &impl HasInitContext) -> InitResult<State> {
    let parameter: InitParameter = ctx.parameter_cursor().get()?;
    Ok(fresh_state(parameter.init_parameter_item, parameter.init_parameter_expiry))
}

/// Receive function in which accounts can bid before the auction end time
#[hax::receive(contract = "auction", name = "bid", payable)]
fn auction_bid<A: HasActions>(
    ctx: &impl HasReceiveContext,
    amount: Amount,
    state: State,
) -> Result<(A, State), BidError> {
    ...
    // state.bids.entry(sender_address).or_insert_with(Amount::zero);
    let bid_to_update : u64 =
        if state.bids.contains_key(&sender_address) {
            state.bids[&sender_address].micro_ccd
        } else {
            0
        };

    bid_to_update += amount.micro_ccd;
    // Ensure that the new bid exceeds the highest bid so far
    ensure!(
        bid_to_update > state.highest_bid.micro_ccd,
        BidError::BidTooLow /* { bid: amount, highest_bid: state.highest_bid } */
    );
    state.highest_bid = Amount {micro_ccd: bid_to_update };
...
#[hax::receive(contract = "auction", name = "finalize")]
