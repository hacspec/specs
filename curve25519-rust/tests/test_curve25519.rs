...
fn test_kat3() {
    for kat in KAT.iter() {
        let s: [u8; 32] = hex::decode(kat.0).unwrap().try_into().unwrap();
        let u: [u8; 32] = hex::decode(kat.1).unwrap().try_into().unwrap();
        let expected: [u8; 32] = hex::decode(kat.2).unwrap().try_into().unwrap();
