...
let schedule (block: array u8 64sz) : array u32 64sz =
  ...
  let s:array u32 64sz = Hax.Array.repeat 0ul 64sz in
  let s:array u32 64sz =
    Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter ({
      ...
      (fun s i ->
          if i <. 16sz
          then
            let s:array u32 64sz = s.[ i ] <- b.[ i ] in
            s
          else
  ...
  let h:array u32 8sz =
    Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter ({
      ...
      (fun h i ->
          ...
          let h:array u32 8sz = h.[ 0sz ] <- Core.Num.wrapping_add_under_impl_8 t1 t2 in
          let h:array u32 8sz = h.[ 1sz ] <- a0 in
          let h:array u32 8sz = h.[ 2sz ] <- b0 in
          let h:array u32 8sz = h.[ 3sz ] <- c0 in
          let h:array u32 8sz = h.[ 4sz ] <- Core.Num.wrapping_add_under_impl_8 d0 t1 in
          let h:array u32 8sz = h.[ 5sz ] <- e0 in
          let h:array u32 8sz = h.[ 6sz ] <- f0 in
          let h:array u32 8sz = h.[ 7sz ] <- g0 in
          h)
  in
  h
...
let compress (block: array u8 64sz) (h_in: array u32 8sz) : array u32 8sz =
  ...
  let h:array u32 8sz =
    Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter ({
      ...
      h
      (fun h i -> h.[ i ] <- Core.Num.wrapping_add_under_impl_8 h.[ i ] h_in.[ i ])
  in
  h

let u32s_to_be_bytes (state: array u32 8sz) : array u8 32sz =
  let (out: array u8 32sz):array u8 32sz = Hax.Array.repeat 0uy 32sz in
...
let hash (msg: slice u8) : array u8 32sz =
  ...
  let (last_block: array u8 64sz):array u8 64sz = Hax.Array.repeat 0uy 64sz in
          ...
          then
            let last_block:array u8 64sz =
              Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter
                ...
                last_block
                (fun last_block i -> last_block.[ i ] <- block.[ i ])
            in
            ...
            h, last_block, last_block_len)
  in
  let last_block:array u8 64sz = last_block.[ last_block_len ] <- 128uy in
    ...
    then
      let last_block:array u8 64sz =
        Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter ({
          ...
          last_block
          (fun last_block i ->
              last_block.[ v_BLOCK_SIZE -. v_LEN_SIZE +. i ] <- len_bist_bytes.[ i ])
      in
      let h:array u32 8sz = compress last_block h in
      h, last_block
    else
      let (pad_block: array u8 64sz):array u8 64sz = Hax.Array.repeat 0uy 64sz in
      let pad_block:array u8 64sz =
        Core.Iter.Traits.Iterator.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter ({
          ...
          pad_block
          (fun pad_block i -> pad_block.[ v_BLOCK_SIZE -. v_LEN_SIZE +. i ] <- len_bist_bytes.[ i ])
      in
      let h:array u32 8sz = compress last_block h in
