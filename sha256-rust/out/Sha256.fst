...
let hash (msg: Alloc.Vec.vec_t UInt8.t Alloc.Alloc.global_t)
    : x: Prims.list UInt8.t {Prims.op_Equality (FStar.List.Tot.Base.length x) 32} =
  ...
  let h, last_block, last_block_len:(x:
    ...
    uint_size) =
    Core.Iter.Iterator.fold (Core.Iter.Traits.Collect.IntoIterator.into_iter (Core.Slice.chunks (Core.Ops.Deref.Deref.deref
                  msg)
              bLOCK_SIZE))
  ...
  in
  let len_bist:UInt64.t = cast (Alloc.Vec.len msg * 8) in
...
let sha256 (msg: Alloc.Vec.vec_t UInt8.t Alloc.Alloc.global_t)
