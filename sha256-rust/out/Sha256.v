...
Notation OpTableType_t := (nseq int8 TODO: Int.to_string length).

Notation Sha256Digest_t := (nseq int8 TODO: Int.to_string length).

Notation RoundConstantsTable_t := (nseq int32 TODO: Int.to_string length).

Notation Hash_t := (nseq int32 TODO: Int.to_string length).
...
Definition OP_TABLE : nseq int8 TODO: Int.to_string length :=
...
Definition K_TABLE : nseq int32 TODO: Int.to_string length :=
...
Definition HASH_INIT : nseq int32 TODO: Int.to_string length :=
...
Definition sigma (x : int32) (i : uint_size) (op : uint_size) : int32 :=
  let tmp := (rotate_right x (into (index OP_TABLE (((@repr WORDSIZE32 3).*i).+(@repr WORDSIZE32 2))))) : int32 in
  let tmp := (if
      op=.?(@repr WORDSIZE32 0)
    then
      x shift_right (index OP_TABLE (((@repr WORDSIZE32 3).*i).+(@repr WORDSIZE32 2)))
    else
      tmp) : int32 in
  let rot_val_1 := (into (index OP_TABLE ((@repr WORDSIZE32 3).*i))) : int32 in
  let rot_val_2 := (into (index OP_TABLE (((@repr WORDSIZE32 3).*i).+(@repr WORDSIZE32 1)))) : int32 in
  ((rotate_right x rot_val_1).^(rotate_right x rot_val_2)).^tmp.

Definition to_be_u32s (block : nseq int8 TODO: Int.to_string length) : Vec_t (int32) (Global_t) :=
  let out := (with_capacity (BLOCK_SIZE./(@repr WORDSIZE32 4))) : Vec_t (int32) (Global_t) in
  let out := (fold (into_iter (chunks (unsize block) (@repr WORDSIZE32 4))) out (fun block_chunk out =>
      let block_chunk_array := (from_be_bytes (unwrap (try_into block_chunk))) : int32 in
      let out := (push out block_chunk_array) : Vec_t (int32) (Global_t) in
      out)) : Vec_t (int32) (Global_t) in
  out.

Definition schedule (block : nseq int8 TODO: Int.to_string length) : nseq int32 TODO: Int.to_string length :=
  let b := (to_be_u32s block) : Vec_t (int32) (Global_t) in
  let s := (repeat (@repr WORDSIZE32 0) (@repr WORDSIZE32 64)) : Box_t (nseq int32 TODO: Int.to_string length) (Global_t) in
  let s := (fold (into_iter (Build_Range_t (@repr WORDSIZE32 0)K_SIZE)) s (fun i s =>
      ...
      then
        ...
        let t16 := (index s (i.-(@repr WORDSIZE32 16))) : int32 in
        let t15 := (index s (i.-(@repr WORDSIZE32 15))) : int32 in
        let t7 := (index s (i.-(@repr WORDSIZE32 7))) : int32 in
        let t2 := (index s (i.-(@repr WORDSIZE32 2))) : int32 in
        ...
        let s := (update_at s i (wrapping_add (wrapping_add (wrapping_add s1 t7) s0) t16)) : nseq int32 TODO: Int.to_string length in
...
Definition shuffle (ws : nseq int32 TODO: Int.to_string length) (hashi : nseq int32 TODO: Int.to_string length) : nseq int32 TODO: Int.to_string length :=
  ...
  let h := (fold (into_iter (Build_Range_t (@repr WORDSIZE32 0)K_SIZE)) h (fun i h =>
      let a0 := (index h (@repr WORDSIZE32 0)) : int32 in
      let b0 := (index h (@repr WORDSIZE32 1)) : int32 in
      let c0 := (index h (@repr WORDSIZE32 2)) : int32 in
      let d0 := (index h (@repr WORDSIZE32 3)) : int32 in
      let e0 := (index h (@repr WORDSIZE32 4)) : int32 in
      let f0 := (index h (@repr WORDSIZE32 5)) : int32 in
      let g0 := (index h (@repr WORDSIZE32 6)) : int32 in
      let h0 := (index h (@repr WORDSIZE32 7)) : int32 in
      let t1 := (wrapping_add (wrapping_add (wrapping_add (wrapping_add h0 (sigma e0 (@repr WORDSIZE32 1) (@repr WORDSIZE32 1))) (ch e0 f0 g0)) (index K_TABLE i)) (index ws i)) : int32 in
      let t2 := (wrapping_add (sigma a0 (@repr WORDSIZE32 0) (@repr WORDSIZE32 1)) (maj a0 b0 c0)) : int32 in
      let h := (update_at h (@repr WORDSIZE32 0) (wrapping_add t1 t2)) : nseq int32 TODO: Int.to_string length in
      ...
      let h := (update_at h (@repr WORDSIZE32 4) (wrapping_add d0 t1)) : nseq int32 TODO: Int.to_string length in
...
Definition compress (block : nseq int8 TODO: Int.to_string length) (h_in : nseq int32 TODO: Int.to_string length) : nseq int32 TODO: Int.to_string length :=
  ...
  let h := (fold (into_iter (Build_Range_t (@repr WORDSIZE32 0)(@repr WORDSIZE32 8))) h (fun i h =>
      update_at h i (wrapping_add (index h i) (index h_in i)))) : nseq int32 TODO: Int.to_string length in
  h.

Definition u32s_to_be_bytes (state : nseq int32 TODO: Int.to_string length) : nseq int8 TODO: Int.to_string length :=
  let out := (repeat (@repr WORDSIZE8 0) (@repr WORDSIZE32 32)) : Box_t (nseq int8 TODO: Int.to_string length) (Global_t) in
  let out := (fold (into_iter (Build_Range_t (@repr WORDSIZE32 0)LEN_SIZE)) out (fun i out =>
      let tmp := (index state i) : int32 in
      let tmp := (to_be_bytes tmp) : nseq int8 TODO: Int.to_string length in
...
Definition hash (msg : Vec_t (int8) (Global_t)) : nseq int8 TODO: Int.to_string length :=
  let h := (HASH_INIT) : nseq int32 TODO: Int.to_string length in
  let last_block := (repeat (@repr WORDSIZE8 0) (@repr WORDSIZE32 64)) : Box_t (nseq int8 TODO: Int.to_string length) (Global_t) in
  let last_block_len := ((@repr WORDSIZE32 0)) : uint_size in
  let '(h,last_block,last_block_len) := (fold (into_iter (chunks (deref msg) BLOCK_SIZE)) (h,last_block,last_block_len) (fun block '(h,last_block,last_block_len) =>
      if
        (len block)<.?BLOCK_SIZE
      then
        let last_block := (fold (into_iter (Build_Range_t (@repr WORDSIZE32 0)(len block))) last_block (fun i last_block =>
            update_at last_block i (index block i))) : nseq int8 TODO: Int.to_string length in
        let last_block_len := (len block) : uint_size in
        (h,last_block,last_block_len)
      else
        let h := (compress (unwrap (try_into block)) h) : nseq int32 TODO: Int.to_string length in
  ...
  let len_bist := (cast ((len msg).*(@repr WORDSIZE32 8))) : int64 in
  let len_bist_bytes := (to_be_bytes len_bist) : nseq int8 TODO: Int.to_string length in
  let '(h,last_block) := (if
      last_block_len<.?(BLOCK_SIZE.-LEN_SIZE)
    then
      let last_block := (fold (into_iter (Build_Range_t (@repr WORDSIZE32 0)LEN_SIZE)) last_block (fun i last_block =>
          update_at last_block ((BLOCK_SIZE.-LEN_SIZE).+i) (index len_bist_bytes i))) : nseq int8 TODO: Int.to_string length in
      let h := (compress last_block h) : nseq int32 TODO: Int.to_string length in
      (h,last_block)
    else
      let pad_block := (repeat (@repr WORDSIZE8 0) (@repr WORDSIZE32 64)) : Box_t (nseq int8 TODO: Int.to_string length) (Global_t) in
      let pad_block := (fold (into_iter (Build_Range_t (@repr WORDSIZE32 0)LEN_SIZE)) pad_block (fun i pad_block =>
          update_at pad_block ((BLOCK_SIZE.-LEN_SIZE).+i) (index len_bist_bytes i))) : nseq int8 TODO: Int.to_string length in
      let h := (compress last_block h) : nseq int32 TODO: Int.to_string length in
...
Definition sha256 (msg : Vec_t (int8) (Global_t)) : nseq int8 TODO: Int.to_string length :=
  hash msg.
